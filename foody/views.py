...
def snippet_list(request, format=None):
    ...
    if request.method == 'GET':
        snippets = Recipe.objects.all().values()
...
def groceryList_list(request, format=None):
    ...
    """
    if request.method == 'GET':
        snippets = GroceryList.objects.all()
        serializer = GroceryListSerializer(snippets, many=True)
        return Response(serializer.data)
        #return JsonResponse(serializer.data, safe=False)
...
def groceryList_detail(request, pk, format=None):
    ...
    """
    try:
        snippet = GroceryList.objects.get(pk=pk)
    except GroceryList.DoesNotExist:
        return Response(status=status.HTTP_404_NOT_FOUND)

    if request.method == 'GET':
        serializer = GroceryListSerializer(snippet)
        return Response(serializer.data)

    elif request.method == 'PUT':
        serializer = GroceryListSerializer(snippet, data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
